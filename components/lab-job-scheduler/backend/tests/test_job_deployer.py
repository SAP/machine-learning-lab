...
class TestJobDeployerWithoutLastRun:
    def test_is_job_due_before_creation_of_job(self, job_without_last_run: ScheduledJob):
        assert not jd.is_due(job_without_last_run, datetime.datetime(
    ...
    def test_is_job_due_on_creation_of_job(self, job_without_last_run: ScheduledJob):
        assert not jd.is_due(job_without_last_run, datetime.datetime(
            2021, 1, 1, hour=0, minute=0, microsecond=0))

    def test_is_job_due_before_next_scheduled_run_time(self, job_without_last_run: ScheduledJob):
        assert not jd.is_due(job_without_last_run, datetime.datetime(
            2021, 1, 1, hour=0, minute=59, second=59, microsecond=0))

    def test_is_job_due_at_next_scheduled_run_time(self, job_without_last_run: ScheduledJob):
        assert jd.is_due(job_without_last_run, datetime.datetime(
            2021, 1, 1, hour=1, minute=0, second=0, microsecond=0))

    def test_is_job_due_after_next_scheduled_run_time(self, job_without_last_run: ScheduledJob):
        assert jd.is_due(job_without_last_run, datetime.datetime(
...
class TestJobDeployerWithLastRun:
    def test_is_job_due_at_last_run_time(self, job_with_last_run: ScheduledJob):
        assert not jd.is_due(job_with_last_run, datetime.datetime(
            2021, 1, 1, hour=1, minute=0, second=0, microsecond=0))
    ...
    def test_is_job_due_before_next_scheduled_run_time(self, job_with_last_run: ScheduledJob):
        assert not jd.is_due(job_with_last_run, datetime.datetime(
            2021, 1, 2, hour=0, minute=59, second=59, microsecond=0))

    def test_is_job_due_at_next_scheduled_run_time(self, job_with_last_run: ScheduledJob):
        assert jd.is_due(job_with_last_run, datetime.datetime(
            2021, 1, 2, hour=1, minute=0, second=0, microsecond=0))

    def test_is_job_due_after_next_scheduled_run_time(self, job_with_last_run: ScheduledJob):
        assert jd.is_due(job_with_last_run, datetime.datetime(
