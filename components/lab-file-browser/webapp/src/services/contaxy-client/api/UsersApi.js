...
export default class UsersApi {
  ...
  /**
   * Get a user token.
   * Returns an API token with permission to access all resources accesible by the given user.  The `read` access level allows read-only access on all resources. The `write` access level allows to create and delete user resources. The `admin` access level allows additional user actions such as deletion of the user itself.
   * @param {String} userId A valid user ID.
