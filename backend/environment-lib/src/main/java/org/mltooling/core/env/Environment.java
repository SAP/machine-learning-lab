...
public class Environment {
  ...
  private void init() {
    ...
    if (!StringUtils.isNullOrEmpty(SystemUtils.getEnvVar(ENV_NAME_LAB_ENDPOINT))
        && !StringUtils.isNullOrEmpty(SystemUtils.getEnvVar(ENV_NAME_LAB_PROJECT))) {
      log.info(
          "Lab endpoint and project were configured via environment variables. Initializing"
              + " environment with lab API.");
      init(
          SystemUtils.getEnvVar(ENV_NAME_LAB_PROJECT),
  ...
  private void init(@Nullable String project, @Nullable LabApiClient labApiClient) {
    ...
    if (labApiClient != null && labApiClient.isAvailable()) {
      ...
      try {
        SingleValueFormat<LabUser> labUser =
            new LabAuthApiClient(labApiClient.getServiceUrl(), labApiClient.getAuthToken()).getMe();
        if (labUser == null
            || !labUser.isSuccessful()
            || labUser.getData() == null
            || StringUtils.isNullOrEmpty(labUser.getData().getId())) {
          log.warn(
              "Failed to get user information from Lab Instance. Initializing local environment");
          connected = false;
        ...
        log.warn(
            "Failed to get user information from Lab Instance. Initializing local environment", e);
        connected = false;
        this.operator = LOCAL_OPERATOR;
      }
    }
  ...
  public void printInfo() {
    ...
    if (isConnected()) {
      log.info(
          "Available datasets: "
              + String.valueOf(getFileHandler().listRemoteFiles(LabFileDataType.DATASET)));
      log.info(
          "Available models: "
              + String.valueOf(getFileHandler().listRemoteFiles(LabFileDataType.MODEL)));
    }
  }
