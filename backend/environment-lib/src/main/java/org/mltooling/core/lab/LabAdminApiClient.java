...
public class LabAdminApiClient extends AbstractApiClient<LabAdminApiClient> implements LabAdminApi {
    ...
    public StatusMessageFormat checkWorkspace(String workspaceId) {
        return executeRequest(Unirest.get(getEndpointUrl() + METHOD_CHECK_WORKSPACE)
                                     .getHttpRequest().queryString(PARAM_WORKSPACE_ID, workspaceId),
                              new TypeToken<StatusMessageFormat>() {}.getType());
    }

    @Override
    public StatusMessageFormat resetAllWorkspaces() {
        return executeRequest(Unirest.put(getEndpointUrl() + METHOD_RESET_ALL_WORKSPACES),
                              new TypeToken<StatusMessageFormat>() {}.getType());
    }
    ...
    public ValueListFormat<LabUser> shutdownUnusedWorkspaces(boolean dryRun, int daysThreshold, @Nullable List<String> includedIds) {
        ...
        return executeRequest(Unirest.put(getEndpointUrl() + METHOD_SHUTDOWN_UNUSED_WORKSPACES)
                                     .queryString(PARAM_DRY_RUN, dryRun)
                                     .queryString(PARAM_DAYS_THRESHOLD, daysThreshold)
                                     .body(new Gson().toJson(includedIds)).getHttpRequest(),
    ...
    public ValueListFormat<String> shutdownDiskExceedingContainers(boolean dryRun) {
        return executeRequest(Unirest.put(getEndpointUrl() + METHOD_SHUTDOWN_DISK_EXCEEDING_CONTAINERS)
                                     .queryString(PARAM_DRY_RUN, dryRun).getHttpRequest(),
                                     new TypeToken<ValueListFormat<String>>() {}.getType());
    }

    @Override
    public SingleValueFormat<LabService> resetWorkspace(String workspaceId) {
        return executeRequest(Unirest.get(getEndpointUrl() + METHOD_RESET_WORKSPACE)
                                     .getHttpRequest().queryString(PARAM_WORKSPACE_ID, workspaceId),
    ...
    public SingleValueFormat<LabInfo> getLabInfo() {
        return executeRequest(Unirest.get(getEndpointUrl() + METHOD_GET_INFO).getHttpRequest(),
                              new TypeToken<SingleValueFormat<LabInfo>>() {}.getType());
    }

    @Override
    public SingleValueFormat<LabProjectsStatistics> getStatistics() {
        return executeRequest(Unirest.get(getEndpointUrl() + METHOD_GET_STATISTICS).getHttpRequest(),
