...
public abstract class AbstractApiClient<T extends AbstractApiClient<T>> {
    ...
    protected HttpResponse<String> executeRequest(HttpRequest request) throws UnirestException {
        ...
        if (!StringUtils.isNullOrEmpty(this.authToken) && StringUtils.isNullOrEmpty(headers.get(ApiUtils.AUTHORIZATION_HEADER))) {
        ...
        log.debug("Executed request: " + request.getHttpMethod().name() + " " + request.getUrl() + " [" + pl.end() + " ms]");
        return response;
    }
    ...
    private String getResponseBody(String requestUrl, HttpResponse<String> response) {
        if (response.getStatus() == HttpStatus.SC_BAD_GATEWAY) {
            log.info("Request for " + requestUrl + " was unsuccessful with status: " + response.getStatusText() +
                             " The service is probably not started yet.");
        } else if (response.getStatus() < HttpStatus.SC_OK || response.getStatus() >= HttpStatus.SC_MULTIPLE_CHOICES) {
            log.info("Request for " + requestUrl + " was unsuccessful with status: " + response.getStatusText() + "("
                             + response.getStatus() + "). Body: " + StringUtils.abbreviate(response.getBody(), 500));
        }
