...
public final class FileUtils {
    ...
    // Initialized with default list from here: http://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types  -> META-INF/mime.types
    ...
    public static List<String> readLinesToList(File file) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(file), StringUtils.UTF_8_CHARSET))) {
    ...
    public static String fileToString(File file) throws IOException {
        ...
        try {
            reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), StringUtils.UTF_8_CHARSET));
            String line = null;
    ...
    public synchronized static void appendToCsv(String filePath, List<String> cells, String outputDelimiter) {
        String fileContent = "";
    ...
    public synchronized static void appendToCsv(String filePath, String fileContent) {
        ...
        if (!file.exists()) {
            ...
            try {
                ...
                fw.write(System.getProperty("line.separator") + fileContent);//appends the string to the file
                fw.close();
            } catch (IOException e) {
    ...
    public synchronized static File stream2File(InputStream in, String tempFilename) {
        try {
    ...
    public synchronized static File stream2File(InputStream in) {
        return stream2File(in, String.valueOf(in.hashCode()));
    }

    public synchronized static void writeStrToFile(String filePath, String fileContent) {
    ...
    public synchronized static void writeStringsToFile(String filePath, Collection<String> fileContent) {
        File file = createFile(filePath);
        if (file == null || fileContent == null) {
            log.warn("File is null " + file.getName());
            return;
     ...
     */
    public synchronized static File copyFile(InputStream is, String targetFilePath, boolean isOverriding) {
    ...
    public static void shuffleFile(String filePath) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(filePath), StringUtils.UTF_8_CHARSET))) {
    ...
    public synchronized static File createOrCleanFile(String filePath) {
        if (StringUtils.isNullOrEmpty(filePath)) {
    ...
    public static String transformEncoding(String text, String sourceEncoding, String targetEncoding) {
        try {
    ...
    private synchronized static File createFile(String filePath) {
        if (StringUtils.isNullOrEmpty(filePath)) {
            log.warn("File path is null or empty");
            return null;
        }
