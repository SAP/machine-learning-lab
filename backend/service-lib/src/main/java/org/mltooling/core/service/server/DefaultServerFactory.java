...
public class DefaultServerFactory {
    ...
    public static void createAndStartServer(ServerConfig serverConfig) {
        ...
        final ResourceConfig rc = new ResourceConfig().register(SystemApiEndpoint.class)
        ...
        HttpServer server = GrizzlyHttpServerFactory.createHttpServer(URI.create(serverConfig.getAddress()), rc);

        // configure api docs swagger gui
        HttpHandler swaggerHandler = new CLStaticHttpHandler(ClassLoader.getSystemClassLoader(), "/swagger/");
        server.getServerConfiguration().addHttpHandler(swaggerHandler, HttpHandlerRegistration.builder().contextPath(serverConfig.getApiDocsEndpoint()).urlPattern("/").build());//serverConfig.getApiDocsEndpoint());
