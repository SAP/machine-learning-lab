...
public class SingleTokenAuthFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext containerRequestContext) throws IOException {
        if (containerRequestContext.getMethod().equalsIgnoreCase(HttpMethod.OPTIONS)
                ...
                Arrays.asList(resourceInfo.getResourceMethod().getAnnotation(RolesAllowed.class).value())
                      .contains(DefaultRoles.PUBLIC))) {
            return;
        }

        String authorizationHeaderValue = cleanToken(containerRequestContext.getHeaders().getFirst("Authorization"));
        if (StringUtils.isNullOrEmpty(authorizationHeaderValue) || !authorizationHeaderValue.contains(cleanedToken)) {
