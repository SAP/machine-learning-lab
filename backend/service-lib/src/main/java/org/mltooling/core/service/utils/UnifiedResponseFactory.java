...
public class UnifiedResponseFactory {
    ...
    public static Response getResponse(UnifiedFormat unifiedFormat, ResponseOptions responseOptions) {
        ...
        return buildResponse(unifiedFormat.getStatus(), responseBody, ApiUtils.CONTENT_TYPE_JSON, responseOptions);
    }
    ...
    public static Response getErrorResponse(Response.Status status, String message) {
        ...
        return buildResponse(status.getStatusCode(), new ErrorMessageFormat(status.getStatusCode(), responseMessage).toJson(), ApiUtils.CONTENT_TYPE_JSON);
    }
    ...
    public static Response getFileDownloadResponse(InputStream in, ContentDisposition contentDisposition) {
    ...
    private Response.ResponseBuilder getCookieResponse(UnifiedFormat unifiedFormat, NewCookie cookie) {
    ...
    private static Response buildResponse(int status, String responseBody, String responseType, ResponseOptions responseOptions) {
        if (StringUtils.isNullOrEmpty(responseBody)) {
            return getEmptyResponse();
        }

        Response.ResponseBuilder responseBuilder = Response.status(status).header(ApiUtils.CONTENT_TYPE_HEADER, responseType + CONTENT_TYPE_CHARSET);
