...
public class PerformanceTests {
  ...
  private static void runTests(PerformanceTests performanceTests) {
    ...
    List<Thread> userAndWorkspaceThreads =
        performanceTests.createUserLoginAndCheckWorkspace(numberUsersWithWorkspaces);
    ...
    new Thread(
            () -> {
              ...
              log.info(
                  "Creating Projects took " + createProjectsPerfLogger.end() + " milliseconds");
            })
        .start();
    ...
    new Thread(
            () -> {
              ...
              log.info(
                  "Creating users and checking workspaces took "
                      + userAndWorkspaceCheckPerfLogger.end()
                      + " milliseconds");
            })
        .start();
  ...
  private void init(String protocol, String host, int port) {
    ...
    adminApiToken =
        new LabAuthApiClient(serviceUrl, adminAppToken).createApiToken(ADMIN_USER).getData();
  ...
  private void checkWorkspaces(int number) {
    for (int i = 0; i < number; i++) {
      ...
      new Thread(
              () -> {
                LabAdminApiClient labAdminApiClient =
                    new LabAdminApiClient(serviceUrl, adminApiToken);
  ...
  private List<Thread> createJob(int number) {
    ...
    for (int i = 0; i < number; i++) {
      Thread t =
          new Thread(
              () ->
                  new LabApiClient(serviceUrl)
                      .deployJob(
                          projectName, "simple-stresstest-job:0.1.8", "Stresstest", "", null));
      t.start();
      threads.add(t);
    }
