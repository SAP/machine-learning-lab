...
public class AdminAuthorizer<U extends CommonProfile> implements Authorizer<U> {
    ...
    public boolean isAuthorized(WebContext context, List<U> profiles) {
        ...
        return !permissions.isEmpty() && (permissions.contains(AuthorizationManager.CORE_PERMISSIONS.ADMIN.getName()) || profile.getId().equalsIgnoreCase(AuthorizationManager.DEFAULT_ADMIN_USER));
