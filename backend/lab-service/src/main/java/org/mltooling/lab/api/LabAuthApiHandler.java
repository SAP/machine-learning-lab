...
public class LabAuthApiHandler extends AbstractApiHandler<LabAuthApiHandler> implements LabAuthApi {
    ...
    // ================ Public Methods ====================================== //
    @Override
    public SingleValueFormat<LabUser> createAdminUser(String user, String password, String jwtSecret) throws UnauthorizedException {
        SingleValueFormat<LabUser> response = new SingleValueFormat<>();

        try {
            if (StringUtils.isNullOrEmpty(user)) {
                response.setErrorStatus("The user parameter should not be empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }

            if (StringUtils.isNullOrEmpty(password)) {
                response.setErrorStatus("The password parameter should not be empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }

            if (StringUtils.isNullOrEmpty(jwtSecret)) {
                response.setErrorStatus("The secret parameter should not be empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }

            // log event
            componentManager.getEventLogManager().logEvent(LabEvent.CREATE_ADMIN_USER, new PropertyContainer()
                    .addProperty("new-user", user)
                    .addProperty(LabAuthApi.PARAM_USER, authProfile != null ? authProfile.getId() : "unk"));

            MongoProfile profile = authManager.createAdminUser(user, password, jwtSecret);
            if (profile == null) {
                response.setErrorStatus("Failed to create admin user profile.", HttpStatus.SC_INTERNAL_SERVER_ERROR);
                return prepareResponse(response);
            }
    ...
    public SingleValueFormat<LabUser> createUser(String user, String password) {
        ...
        try {
            ...
            if (AuthorizationManager.isTechnicalUser(user)) {
                response.setErrorStatus("User name is reserved for technical users.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }

            if (user.length() > 25) {
                // TODO: quick fix, non technical users should not be longer than 25 chars -> otherwise there might be some problems
                response.setErrorStatus("User name is not allowed to be longer than 25 chars.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            MongoProfile profile = authManager.createUser(user, password);
            if (profile == null) {
                response.setErrorStatus("Failed to create user profile.", HttpStatus.SC_INTERNAL_SERVER_ERROR);
                return prepareResponse(response);
            }
    ...
    public SingleValueFormat<String> refreshToken() {
        ...
        try {
            if (this.authProfile == null) {
                response.setErrorStatus("No profile found. Failed to authenticate.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }

            String token = authManager.refreshToken(this.authProfile);
            if (StringUtils.isNullOrEmpty(token)) {
                response.setErrorStatus("Failed to refresh app token.", HttpStatus.SC_INTERNAL_SERVER_ERROR);
                return prepareResponse(response);
            }
    ...
    public SingleValueFormat<LabUser> getMe() {
        ...
        try {
            ...
            if (profile == null) {
                response.setErrorStatus("Failed to get user profile for current user.", HttpStatus.SC_INTERNAL_SERVER_ERROR);
                return prepareResponse(response);
            }
            response.setData(AuthorizationManager.transformProfile(profile));
    ...
    public SingleValueFormat<LabUser> getUser(String user) {
        ...
        try {
            ...
            if (profile == null) {
                response.setErrorStatus("Failed to get user profile for " + user, HttpStatus.SC_INTERNAL_SERVER_ERROR);
                return prepareResponse(response);
            }
            response.setData(AuthorizationManager.transformProfile(profile));
    ...
    public StatusMessageFormat deactivateUsers(List<String> users) {
        ...
        try {
            if (users == null) {
                response.setErrorStatus("The body should contain a list of users.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
    ...
    public StatusMessageFormat deleteUser(String user) {
        ...
        try {
            ...
            componentManager.getEventLogManager().logEvent(LabEvent.DELETE_USER, new PropertyContainer()
    ...
    @Override
    public SingleValueFormat<String> updatePermissions(String user, List<String> permissions, @Nullable Boolean deactivateToken) {
        SingleValueFormat<String> response = new SingleValueFormat<>();
        ...
        try {
            ...
            if (permissions == null) {
                // empty list is fine
                response.setErrorStatus("The permission list in the body should not be empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            ...
            if (AuthorizationManager.isTechnicalUser(user)) {
                ...
                response.setErrorStatus("It is not allowed to update permissions of technical users.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
    ...
    public SingleValueFormat<String> updateUserPassword(String user, String password) {
        ...
        try {
            ...
            if (authProfile != null && AuthorizationManager.isProjectAdmin(authProfile.getId())) {
                response.setErrorStatus("A project admin user is not allowed to update the user password.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
    ...
    public SingleValueFormat<String> addUserToProject(String user, String project) {
        ...
        try {
            ...
            if (AuthorizationManager.isTechnicalUser(user)) {
                response.setErrorStatus("It is not allowed to add technical users to projects.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
    ...
    public SingleValueFormat<String> removeUserFromProject(String user, String project) {
        ...
        try {
            ...
            if (AuthorizationManager.isTechnicalUser(user)) {
                response.setErrorStatus("It is not allowed to remove technical users from projects.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
