...
public class DockerServiceManager extends AbstractServiceManager {
    ...
    private static final DockerUtils.Network INITIAL_CIDR = new DockerUtils.Network(String.format("%d.%d.0.0/24",
    ...
    public List<LabService> getServices(@Nullable String project) throws Exception {
        ...
        try {
            ...
            if (StringUtils.isNullOrEmpty(project)) {
                dockerContainers = client.listContainers(DockerClient.ListContainersParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE));
            } else {
                dockerContainers = client.listContainers(DockerClient.ListContainersParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE),
                                                         DockerClient.ListContainersParam.withLabel(LABEL_FEATURE_TYPE, FeatureType.PROJECT_SERVICE.getName()),
    ...
    public boolean deleteService(String serviceId, boolean removeVolumes, @Nullable String project) throws Exception {
        ...
        log.info("Delete service: " + service.getDockerName() + " (remove volumes: " + removeVolumes + ")");

        // delete service and all volumes
        if (removeVolumes) {
            client.removeContainer(service.getDockerId(), DockerClient.RemoveContainerParam.removeVolumes(true), DockerClient.RemoveContainerParam.forceKill());
        } else {
            // do not force kill to prevent database corruption
            //client.removeContainer(service.getDockerId(), DockerClient.RemoveContainerParam.forceKill());
    ...
    public boolean deleteJob(String jobId, String project) throws Exception {
        ...
        client.removeContainer(job.getDockerId(), DockerClient.RemoveContainerParam.removeVolumes(true), DockerClient.RemoveContainerParam.forceKill());
    ...
    public boolean deleteProjectResources(String project) throws Exception {
        for (Container containers : client.listContainers(DockerClient.ListContainersParam.allContainers(),
                                                          DockerClient.ListContainersParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE),
                                                          DockerClient.ListContainersParam.withLabel(LABEL_PROJECT, project))) {
            //client.killContainer(containers.id());
            client.removeContainer(containers.id(), DockerClient.RemoveContainerParam.removeVolumes(true), DockerClient.RemoveContainerParam.forceKill());
        }
        Thread.sleep(5000);

        VolumeList volumeList = client.listVolumes(DockerClient.ListVolumesParam.filter(FILTER_LABEL, LABEL_PROJECT + "=" + project),
                                                   DockerClient.ListVolumesParam.filter(FILTER_LABEL, LABEL_NAMESPACE + "=" + LabConfig.LAB_NAMESPACE));
        ...
        for (Network network : client.listNetworks(DockerClient.ListNetworksParam.filter(FILTER_LABEL, LABEL_PROJECT + "=" + project),
                                                   DockerClient.ListNetworksParam.filter(FILTER_LABEL, LABEL_NAMESPACE + "=" + LabConfig.LAB_NAMESPACE))) {
            ImmutableMap<String, Network.Container> connectedContainers = client.inspectNetwork(network.id()).containers();
    ...
    public List<LabJob> getJobs(String project) throws Exception {
        ...
        try {
            List<Container> dockerContainers = client.listContainers(DockerClient.ListContainersParam.allContainers(),
                                                                     DockerClient.ListContainersParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE),
                                                                     DockerClient.ListContainersParam.withLabel(LABEL_FEATURE_TYPE, FeatureType.PROJECT_JOB.getName()),
                                                                     DockerClient.ListContainersParam.withLabel(LABEL_PROJECT, project));
    ...
    public void installLab() throws Exception {
        ...
        if (!isServiceHealthy(CoreService.PORTAINER.getName())) {
            log.info(CoreService.PORTAINER.getName() + " service does not exist. Create service based on " + CoreService.PORTAINER.getImage());
            DockerDeploymentConfig portainerConfig = createCoreService(CoreService.PORTAINER.getImage(), CoreService.PORTAINER.getName())
                    ...
                    .addMount(DockerDeploymentConfig.BIND_MOUNT_TYPE, "/var/run/docker.sock:/var/run/docker.sock");
    ...
    protected LabService getLabService() throws Exception {
        String containerId = System.getenv("HOSTNAME");  // HOSTNAME is default in docker to get the container id
    ...
    public void updateLab(boolean backendOnly) throws Exception {
        ...
        for (Network network : client.listNetworks(DockerClient.ListNetworksParam.filter(FILTER_LABEL, LABEL_NAMESPACE + "=" + LabConfig.LAB_NAMESPACE))) {
    ...
    public DockerDeploymentConfig createLabService() {
        ...
        try {
            if (LabConfig.SERVICE_SSL_ENABLED &&
                    client.listVolumes(DockerClient.ListVolumesParam.filter(FILTER_NAME, SSL_VOLUME_NAME)).volumes().size() == 1) {
                // TODO only add volume if container exists? Needs better solution!
                // add ssl volume to backend service if ssl is enabled and if the ssl-volume was manually populated with certificates.
                // if the volume does not exist, do not mount it as the :ro part would prevent the ML Lab container to auto-generate certificates.
                String sslResourcesPath = SystemUtils.getEnvVar("_SSL_RESOURCES_PATH", "/resources/ssl");
                deploymentConfig.addMount(DockerDeploymentConfig.BIND_MOUNT_TYPE, SSL_VOLUME_NAME + ":" + sslResourcesPath + ":ro");
        ...
        try {
            if (client.listVolumes(DockerClient.ListVolumesParam.filter(FILTER_NAME, TOS_VOLUME_NAME)).volumes().size() == 1) {
                // Only mount TOS if volume exists
                deploymentConfig.addMount(DockerDeploymentConfig.BIND_MOUNT_TYPE, TOS_VOLUME_NAME + ":" + LabConfig.TERMS_OF_SERVICE_FOLDER_PATH + ":ro");
            }
        } catch (DockerException | InterruptedException e) {
    ...
    public DockerDeploymentConfig createWorkspaceService(String user) throws Exception {
        ...
        if (!StringUtils.isNullOrEmpty(LabConfig.HOST_ROOT_WORKSPACE_DATA_MOUNT_PATH)) {
            ...
            Path hostPath = new File(LabConfig.HOST_ROOT_WORKSPACE_DATA_MOUNT_PATH).toPath().resolve(workspaceDeployment.getName());

            // set mount path
            workspaceDeployment.addMount(DockerDeploymentConfig.BIND_MOUNT_TYPE, hostPath.toString() + ":" + workspaceDeployment.getVolumePath());
    ...
    public List<String> shutdownDiskExceedingContainers(boolean dryRun) throws Exception {
        ...
        List<Container> containers = client.listContainers(DockerClient.ListContainersParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE),
                                                            DockerClient.ListContainersParam.withLabel(LABEL_FEATURE_TYPE, FeatureType.WORKSPACE.getName()),
                                                            DockerClient.ListContainersParam.withContainerSizes(true));
        client.listContainers();
        for (Container container : containers) {
            // ignore all core services
            // TODO: following check is only needed when the functionality should be extended to all containers, not only workspaces. 
            // But then, re-creation logic has to be finished
            if (container.labels().getOrDefault(LABEL_FEATURE_TYPE, FeatureType.CORE_SERVICE.getName()).equalsIgnoreCase(FeatureType.CORE_SERVICE.getName())) {
                continue;
            }
            ...
            if (container.sizeRw() != null ) {
                ...
                if (LabConfig.MAX_CONTAINER_SIZE != MAX_CONTAINER_SIZE_DISABLED &&
                    ...
                    if (!dryRun) {
                        // Remove the container. Workspace containers are automatically re-created when user tries to use it.
                        ...
                        // TODO: if re-created in this way, somehow the hostConfig settings (e.g. cpu limi etc.) are not considered. Some other way to perserve the settings? Probably call the respective Lab endpoints instead.
                        // ContainerCreation containerCreation = client.createContainer(containerInfo.config(), containerName);
    ...
    public static void cleanUpLab(boolean keepVolumes) throws Exception {
        ...
        Thread.sleep(10000);
        for (Network network : client.listNetworks(DockerClient.ListNetworksParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE))) {
            try {
        ...
        if (!keepVolumes) {
            VolumeList volumeList = client.listVolumes(DockerClient.ListVolumesParam.filter(FILTER_LABEL, LABEL_NAMESPACE + "=" + LabConfig.LAB_NAMESPACE));
    ...
    /**
     * check container's health status by Leveraging Docker's native Health information (either health or normal state).
     *
    ...
    private DockerUtils.ContainerState getContainerStatus(ContainerInfo container) {
        ...
        if (container.state().health() != null) {
            ...
            // the status for containers is one of (starting, healthy, unhealthy, none). Exited containers also have the status 'unhealthy'.
    ...
    protected void connectToNetwork(String dockerId, String networkName) throws DockerException, InterruptedException {
    ...
    protected String deployContainer(DockerDeploymentConfig serviceConfig) throws Exception {
        ...
        if (!StringUtils.isNullOrEmpty(serviceConfig.getVolumePath())) {
            ...
            if (!StringUtils.isNullOrEmpty(LabConfig.HOST_ROOT_DATA_MOUNT_PATH)) {
                ...
                Path hostPath = new File(LabConfig.HOST_ROOT_DATA_MOUNT_PATH).toPath().resolve(serviceConfig.getName());

                hostConfig.appendBinds(HostConfig.Bind.from(hostPath.toString())
                                                      .to(serviceConfig.getVolumePath())
                                                      .readOnly(false)
                                                      .build());
            } else {
                // created named volume
                Volume containerVolume = Volume.builder()
                                               .name(serviceConfig.getName())
                                               .driver("local")
                                               .labels(serviceConfig.getLabels()).build();

                hostConfig.appendBinds(HostConfig.Bind.from(client.createVolume(containerVolume))
                                                      .to(serviceConfig.getVolumePath())
                                                      .readOnly(false)
                                                      .build());
            }
        ...
        // add binds
        if (!ListUtils.isNullOrEmpty(serviceConfig.getMounts().get(DockerDeploymentConfig.BIND_MOUNT_TYPE))) {
            hostConfig.appendBinds(serviceConfig.getMounts().get(DockerDeploymentConfig.BIND_MOUNT_TYPE));
        }

        // add resource limits
        try {
            // cpus to nano cpus https://stackoverflow.com/questions/52398136/how-to-verify-cpu-limit-assigned-to-a-docker-container
        ...
        if (!ListUtils.isNullOrEmpty(serviceConfig.getNetworks())) {
            try {
                ...
                log.warn("Failed to disconnect container " + creation.id() + " from bridge network", e.getMessage());
            }
        }
        ...
        for (String network : serviceConfig.getNetworks()) {
            try {
                ...
                Map<String, String> networkLabels = new HashMap<>();
                if (serviceConfig.getLabels() != null && !StringUtils.isNullOrEmpty(serviceConfig.getLabels().get(LABEL_PROJECT))) {
            ...
            try {
                ...
                log.warn("Could not connect docker container " + creation.id() + " to network " + network, e);
            }
        }
    ...
    /**
     * Create Docker networks by determining whether a new subnet has to be used. Otherwise, the default Docker subnet would be used
     ...
     * @param networkLabels
     */
    protected void createNetwork(String name, @Nullable Map<String, String> networkLabels) throws Exception {
        List<Network> networks = client.listNetworks(); //client.listNetworks(DockerClient.ListNetworksParam.byNetworkName(name))
        ...
        for (Network network : networks) {
            ...
            if (network.ipam() != null
                ...
                DockerUtils.Network cidr = new DockerUtils.Network(network.ipam().config().get(0).subnet()); // e.g.: 172.33.0.1/24
        ...
        try {
            NetworkCreation networkCreation = client.createNetwork(NetworkConfig.builder()
                                                                                .labels(networkLabels)
                                                                                .name(name)
                                                                                .ipam(Ipam.create("default",
    ...
    protected ContainerInfo getContainer(String containerId, @Nullable String project) throws Exception {
        ...
        try {
            ...
            if (StringUtils.isNullOrEmpty(namespace) || !namespace.equalsIgnoreCase(LabConfig.LAB_NAMESPACE)) {
                containerInfo = null; // set to null because the one that was found is not in namespace
                throw new ContainerNotFoundException("Container with name " + containerId + " found but not in the correct namespace " + LabConfig.LAB_NAMESPACE);
            }
            ...
            if (!StringUtils.isNullOrEmpty(project)) {
                ...
                dockerContainers = client.listContainers(DockerClient.ListContainersParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE),
                                                         ...
                                                         DockerClient.ListContainersParam.withLabel(LABEL_PROJECT, ProjectManager.processNameToId(project)));
            } else {
                dockerContainers = client.listContainers(DockerClient.ListContainersParam.withLabel(LABEL_NAMESPACE, LabConfig.LAB_NAMESPACE),
                                                         DockerClient.ListContainersParam.withLabel(LABEL_FEATURE_NAME, containerId));
            }
        ...
        if (!StringUtils.isNullOrEmpty(project)) {
            ...
            if (StringUtils.isNullOrEmpty(serviceProject)) {
                throw new Exception("Container was found for " + containerId + " but does not has a project attached.");
            }
            if (!serviceProject.equalsIgnoreCase(project)) {
                throw new Exception("Container was found for " + containerId + " but is part of different project: " + serviceProject);
            }
        }
    ...
    private LabService transformService(ContainerInfo container) {
        ...
        LabService mlService = new LabService()
                .setName(featureName)
                .setDockerName(containerName)
                .setDockerId(container.id())
                .setDockerImage(containerImage)
    ...
    @Deprecated
    private Integer getPublishedPort(Integer internalPort, Map<String, List<PortBinding>> portMappings) {
