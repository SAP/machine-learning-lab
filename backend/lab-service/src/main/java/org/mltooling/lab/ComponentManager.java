...
public enum ComponentManager {
    ...
    /**
     * Get a singleton instance of the respective ServiceManager, determined by {@link LabConfig#SERVICES_RUNTIME}.
     * The
     * <a href="https://github.com/spotify/docker-client/blob/master/docs/user_manual.md#connection-pooling">
     * official documentation
     * </a>
     * states that a DockerClient can have multiple concurrent requests, hence it should be threadsafe.
     * Creating a Docker client on each request is expensive.
     *
    ...
    public FileStorageManager getFileManager() {
        if (fileStorageManager == null) {
            try {
                ...
                if (LabConfig.ENV_S3_ENDPOINT != null && LabConfig.ENV_S3_ACCESS_KEY != null && LabConfig.ENV_S3_SECRET_KEY != null) {
                    s3Endpoint = LabConfig.ENV_S3_ENDPOINT;
                    s3AccessKey = LabConfig.ENV_S3_ACCESS_KEY;
                    s3SecretKey = LabConfig.ENV_S3_SECRET_KEY;
                    s3Secured = (LabConfig.ENV_S3_SECURED == null) ? false : Boolean.valueOf(LabConfig.ENV_S3_SECURED);
                } else {
                    LabService minioService = getServiceManger().getService(CoreService.MINIO.getName());
                    s3Endpoint = "http://" + minioService.getDockerName() + ":" + minioService.getConnectionPort();
                    s3AccessKey = AbstractServiceManager.DEFAULT_SERVICE_USER;
                    s3SecretKey = AbstractServiceManager.DEFAULT_SERVICE_PASSWORD;
                    s3Secured = false;
                }

                this.fileStorageManager = new FileStorageManager(s3Endpoint, s3AccessKey, s3SecretKey, s3Secured);
            } catch (Exception e) {
    ...
    public MongoDbManager getMongoManager() {
        if (mongoDbManager == null) {
            try {
                ...
                mongoDbManager = new MongoDbManager(mongoService.getDockerName(), mongoService.getConnectionPort(), false, AbstractServiceManager.DEFAULT_SERVICE_USER, AbstractServiceManager.DEFAULT_SERVICE_PASSWORD);
            } catch (Exception e) {
    ...
    public boolean isLocalRuntime() {
        ...
        return StringUtils.isNullOrEmpty(mode) || mode.equalsIgnoreCase("local") || mode.equalsIgnoreCase("docker");
    }
