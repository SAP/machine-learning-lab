...
public class EventLogManager {
    ...
    public void logEvent(String name, Map<String, Object> attributes) {
        ...
        if (loggingActivated) {
            getEventsCollection().insertOne(new Document(EVENTS_COLLECTION_PROPERTIES.NAME.value, name)
    ...
    public List<LabEvent> getEvents(@Nullable String eventName) {
        ...
        if (StringUtils.isNullOrEmpty(eventName)) {
            ...
            dbEvents = getEventsCollection().find(new Document(new Document(EVENTS_COLLECTION_PROPERTIES.NAME.value, eventName)));
        }
    ...
    private LabEvent transformMongoDocument(Document projectDoc) {
        return new LabEvent().setName(projectDoc.getString(EVENTS_COLLECTION_PROPERTIES.NAME.value))
