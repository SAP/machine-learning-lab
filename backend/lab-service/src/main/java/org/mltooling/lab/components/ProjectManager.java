...
public class ProjectManager {
  ...
  private static final String PROJECT_NAME_VALIDATION_REGEX =
      "^[a-zA-Z0-9][a-zA-Z0-9\\-\\s]{1,33}[a-zA-Z0-9]$"; // min 3 chars max 35 chars
  // bucket restrictions:
  ...
  public void createProject(LabProjectConfig projectConfig) throws Exception {
    if (!isValidProjectName(projectConfig.getName())) {
      throw new Exception(
          "The project name is not valid. It may only contain letters, digits, spaces, and the"
              + " minus-character. The name also has to be between 3 to 35 characters.");
    }
    ...
    if (projectConfig.getName().startsWith(AuthorizationManager.USER_PROJECT_PREFIX)) {
      throw new Exception(
          "Project name should not start with " + AuthorizationManager.USER_PROJECT_PREFIX);
    }

    if (projectExists(projectId)) {
      throw new Exception(
          "Project " + projectConfig.getName() + " already exists, please try another name.");
    }
    ...
    mongoDbManager
        ...
        .insertOne(
            new Document(PROJECTS_COLLECTION_PROPERTIES.ID.value, projectId)
                .append(PROJECTS_COLLECTION_PROPERTIES.NAME.value, projectConfig.getName())
                .append(
                    PROJECTS_COLLECTION_PROPERTIES.VISIBILITY.value,
                    projectConfig.getVisibility().getName())
                ...
                .append(
                    PROJECTS_COLLECTION_PROPERTIES.DESCRIPTION.value,
    ...
    if (project.startsWith(AuthorizationManager.USER_PROJECT_PREFIX)) {
      throw new Exception(
          "The project name is not valid. "
              + AuthorizationManager.USER_PROJECT_PREFIX
              + " prefix is reserved for special user projects.");
    }
  ...
  /**
   * Get all members belonging to {@code project} //TODO: The implementation might be slow, due to
   * iterating over all users, deserializing, and then returning them. This is because currently the
   * project a user belongs to is only stored in the serialized profie
   *
   * @param project for which the members should be fetched
   * @return a list of all members. LabUser will only contain information which every user is
   *     allowed to see
   * @throws Exception
   */
  public List<String> getProjectMembers(String project) throws Exception {
    ...
    FindIterable<Document> profileIterator =
        mongoDbManager.getLabMongoDb().getCollection(USERS_COLLECTION_NAME).find();
  ...
  private LabProject transformMongoDocument(Document projectDoc) {
    return new LabProject()
        .setId(projectDoc.getString(PROJECTS_COLLECTION_PROPERTIES.ID.value))
  ...
  private boolean isValidProjectName(String projectName) {
    return !StringUtils.isNullOrEmpty(projectName)
        && projectName.matches(PROJECT_NAME_VALIDATION_REGEX);
  }

  private MongoCollection<Document> getProjectsCollection() {
    return mongoDbManager.getLabMongoDb().getCollection(PROJECTS_COLLECTION);
  }

  private boolean projectExists(String project) {
    try {
      return getProjectsCollection()
              .countDocuments(new Document(PROJECTS_COLLECTION_PROPERTIES.ID.value, project))
          > 0;
    } catch (Exception e) {
