...
public abstract class AbstractServiceManager {
    ...
    public static final String SERVICE_NAME_VALIDATION_REGEX = "[a-zA-Z0-9][a-zA-Z0-9\\-]+[a-zA-Z0-9]";
     ...
     */
    public abstract LabService deployService(DockerDeploymentConfig deploymentConfig) throws Exception;
     ...
     */
    public abstract LabService getService(String serviceId, @Nullable String project) throws Exception;

    /**
     * Delete a specific service
     */
    public abstract boolean deleteService(String serviceId, boolean removeVolumes, @Nullable String project) throws Exception;
    ...
    public void updateLab(boolean backendOnly) throws Exception {
        log.info("Updating Lab (namespace: " + LabConfig.LAB_NAMESPACE + ") to version " + LabConfig.SERVICE_VERSION);
        ...
        if (backendService != null) {
            ...
            if (!StringUtils.isNullOrEmpty(namespace) && !namespace.equals(LabConfig.LAB_NAMESPACE)) {
                throw new Exception(LabConfig.ENV_NAME_LAB_NAMESPACE + " should be the same value as the one of the current Lab instance: " + namespace);
            }
            ...
            if (!StringUtils.isNullOrEmpty(runtime) && !runtime.equals(LabConfig.SERVICES_RUNTIME)) {
                throw new Exception(LabConfig.ENV_NAME_SERVICES_RUNTIME + " should be the same value as the one of the current Lab instance: " + runtime);
            }

            if (backendOnly) {
                // only check these value if the light-update is selected = allow changing parameters in update-full
                ...
                if (!StringUtils.isNullOrEmpty(jwtToken) && !jwtToken.equals(LabConfig.JWT_SECRET)) {
                    throw new Exception(LabConfig.ENV_NAME_JWT_SECRET + " should be the same value as the one of the current Lab instance: " + jwtToken);
                }

                String sslEnabled = backendService.getConfiguration().get(LabConfig.ENV_NAME_SSL_ENABLED);
                if (!StringUtils.isNullOrEmpty(sslEnabled) && !Boolean.valueOf(sslEnabled) == LabConfig.SERVICE_SSL_ENABLED) {
                    throw new Exception(LabConfig.ENV_NAME_SSL_ENABLED + " should be the same value as the one of the current Lab instance: " + sslEnabled);
                }

                String hostRootMount = backendService.getConfiguration().get(LabConfig.ENV_NAME_HOST_ROOT_DATA_MOUNT_PATH);
                if ((StringUtils.isNullOrEmpty(hostRootMount) && !StringUtils.isNullOrEmpty(LabConfig.HOST_ROOT_DATA_MOUNT_PATH)) ||
                        (!StringUtils.isNullOrEmpty(hostRootMount) && !hostRootMount.equals(LabConfig.HOST_ROOT_DATA_MOUNT_PATH))) {
                    throw new Exception(LabConfig.ENV_NAME_HOST_ROOT_DATA_MOUNT_PATH + " should be the same value as the one of the current Lab instance: " + hostRootMount);
                }

                String hostWorkspaceMount = backendService.getConfiguration().get(LabConfig.ENV_NAME_HOST_ROOT_WORKSPACE_DATA_MOUNT_PATH);
                if ((StringUtils.isNullOrEmpty(hostWorkspaceMount) && !StringUtils.isNullOrEmpty(LabConfig.HOST_ROOT_WORKSPACE_DATA_MOUNT_PATH)) ||
                        (!StringUtils.isNullOrEmpty(hostWorkspaceMount) && !hostWorkspaceMount.equals(LabConfig.HOST_ROOT_WORKSPACE_DATA_MOUNT_PATH))) {
                    throw new Exception(LabConfig.ENV_NAME_HOST_ROOT_WORKSPACE_DATA_MOUNT_PATH + " should be the same value as the one of the current Lab instance: " + hostWorkspaceMount);
                }
            }
    ...
    public LabService checkWorkspace(String user) throws Exception {
        ...
        // wrap around try-catch because underlying method 'getService' throws an exception when the defined service is not found
        try {
    ...
    public void installLab() throws Exception {
        ...
        if (!isServiceHealthy(CoreService.MINIO.getName())) {
            log.info(CoreService.MINIO.getName() + " service does not exist. Create service based on " + CoreService.MINIO.getImage());
            // TODO set username and password
            DockerDeploymentConfig minioConfig = createCoreService(CoreService.MINIO.getImage(), CoreService.MINIO.getName())
        ...
        if (!isServiceHealthy(CoreService.MONGO.getName())) {
            log.info(CoreService.MONGO.getName() + " service does not exist. Create service based on " + CoreService.MONGO.getImage());

            DockerDeploymentConfig mongoConfig = createCoreService(CoreService.MONGO.getImage(), CoreService.MONGO.getName())
        ...
        if (!isServiceHealthy(CoreService.LAB_BACKEND.getName())) {
            log.info(CoreService.LAB_BACKEND.getName() + " service does not exist. Create service based on " + CoreService.LAB_BACKEND.getImage());
    ...
    public DockerDeploymentConfig createWorkspaceService(String user) throws Exception {
        ...
        envVars.put(ENV_NAME_LAB_API_TOKEN, ComponentManager.INSTANCE.getAuthManager().createApiToken(user));
        ...
        envVars.put("WORKSPACE_STORAGE_LIMIT", LabConfig.SERVICES_STORAGE_LIMIT); // 100 GB limit on /workspace folder
        ...
        envVars.put("SSH_JUMPHOST_TARGET", dockerName); // same like Workspace Name, but will be used by Workspace Container to differentiate between setup with or without runtime manager
        ...
        DockerDeploymentConfig workspaceConfig = new DockerDeploymentConfig()
                .setName(dockerName)
    ...
    public DockerDeploymentConfig createLabService() {
        ...
        DockerDeploymentConfig deploymentConfig = createCoreService(CoreService.LAB_BACKEND.getImage(), CoreService.LAB_BACKEND.getName())
    ...
    public DockerDeploymentConfig createCoreService(String image, @Nullable String name) {
        ...
        // envVars.put(ENV_NAME_LAB_API_TOKEN, ComponentManager.INSTANCE.getAuthManager().createAdminToken());

        DockerDeploymentConfig config = new DockerDeploymentConfig()
                .setName(dockerName)
                .setImage(image)
                .setNamespace(LabConfig.LAB_NAMESPACE)
     ...
     */
    public DockerDeploymentConfig createProjectService(String project, String image, @Nullable String name) {
        ...
        String dockerName = processDockerName(LabConfig.LAB_NAMESPACE + "-" + project + "-" + name.toLowerCase());

        Map<String, String> envVars = new HashMap<>();
        envVars.put(ENV_NAME_LAB_ENDPOINT, this.getLabBackendEndpoint());
        try {
            envVars.put(ENV_NAME_LAB_API_TOKEN, ComponentManager.INSTANCE.getAuthManager().createProjectToken(project));
        } catch (Exception e) {
            log.error("Failed to create project token.", e);
        }

        envVars.put(ENV_NAME_LAB_PROJECT, project);
        envVars.put(ENV_NAME_LAB_SERVICE_PATH, LabConfig.LAB_BASE_URL + "/lab/projects/" + project + "/services/" + dockerName + "/");
        ...
        DockerDeploymentConfig deploymentConfig = new DockerDeploymentConfig()
                .setName(dockerName)
                .setImage(image)
                .setNamespace(LabConfig.LAB_NAMESPACE)
     ...
     */
    public DockerDeploymentConfig createProjectJob(String project, String image, @Nullable String name) {
        // Project should be already checked to be valid
        ...
        String dockerName = processDockerName(LabConfig.LAB_NAMESPACE + "-" + project + "-" + name.toLowerCase(), DOCKER_NAME_MAX_LENGTH - 14);

        // Add timestamp to name to make the name unique; otherwise, only a single job per image & project could exist.
    ...
    public String getLabBackendEndpoint() {
        ...
        return protocol + "://" + LabConfig.LAB_NAMESPACE + "-"
    ...
    public boolean isValidServiceName(String serviceName) {
        return !StringUtils.isNullOrEmpty(serviceName) && serviceName.matches(SERVICE_NAME_VALIDATION_REGEX);
    }
    ...
    protected Set<Integer> getExposedPortsFromImage(String dockerImage) {
        ...
        try {
            ...
            if (imageDetails != null && imageDetails.config() != null && imageDetails.config().exposedPorts() != null) {
                for (String port : imageDetails.config().exposedPorts()) {
                    ports.add(Integer.valueOf(port.split("/")[0]));
                }
                return ports;
            } else {
                log.info("Failed to inspect image: " + dockerImage + ". Probably does not have any exposed ports.");
                return ports;
            }
    ...
    protected String getWorkspaceName(String user) {
        return processDockerName(LabConfig.LAB_NAMESPACE + "-" + CoreService.WORKSPACE.getName() + "-" + user);
    }
        ...
        try {
            while (!getService(serviceName).getIsHealthy()) {
                ...
                if (waitTime >= maxWaitTime) {
                    log.info("Service " + serviceName + " isn't available after " + TimeUnit.MILLISECONDS.toMinutes(waitTime) + " min wait time.");
                    return false;
                }
            }
        } catch (Exception e) {
    ...
    protected ImageInfo inspectImage(String dockerImage) throws DockerException, InterruptedException {
        ...
        if (!StringUtils.isNullOrEmpty(imageVersion) &&
                (imageVersion.equalsIgnoreCase("latest"))) { //  || imageVersion.toUpperCase().endsWith("SNAPSHOT")
            try {
