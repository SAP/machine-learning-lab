...
public class LabAdminApiHandler extends AbstractApiHandler<LabAdminApiHandler> implements LabAdminApi {
    ...
    public StatusMessageFormat resetAllWorkspaces() {
        ...
        try {
            // log event
            componentManager.getEventLogManager().logEvent(LabEvent.RESET_ALL_WORKSPACES, new PropertyContainer()
     ...
     */
    @Override
    public ValueListFormat<LabUser> shutdownUnusedWorkspaces(boolean dryRun, int daysThreshold, @Nullable List<String> includedIds) {
        ValueListFormat<LabUser> response = new ValueListFormat<>();

        try {
            List<LabUser> inactiveUsers = componentManager.getAuthManager().getInactiveUsers(daysThreshold);
                ...
                for (LabUser user : inactiveUsers) {
                    try {
                        if (user.getLastActivity() != null) {
                            long inactiveSince = TimeUnit.MILLISECONDS.toDays(new Date().getTime() - user.getLastActivity().getTime());
                            log.info("Shutdown workspace for " + user.getName() + "; Inactive since (days): " + inactiveSince);
                        }
            ...
            // log event
            componentManager.getEventLogManager().logEvent(LabEvent.SHUTDOWN_UNUSED_WORKSPACES, new PropertyContainer()
                    .addProperty("user", authProfile != null ? authProfile.getId() : "unk")
     ...
     *
     * @param user - id which should be used to identify the container. Will be prefixed with {WORKSPACE_SERVICE_PREFIX}
     */
    public SingleValueFormat<LabService> resetWorkspace(String user) {
        ...
        try {
            if (StringUtils.isNullOrEmpty(user)) {
                response.setErrorStatus("The workspace id (user) parameter is empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            ...
            if (this.authProfile == null || user == null) {
                response.setErrorStatus("Not authorized to reset workspace: " + user, HttpStatus.SC_UNAUTHORIZED);
                return prepareResponse(response);
            }

            if (!this.authProfile.getId().equalsIgnoreCase(user) && !componentManager.getAuthManager().isAdmin(this.authProfile)) {
                response.setErrorStatus("User " + this.authProfile.getId() + " is not allowed to reset workspace: " + user, HttpStatus.SC_UNAUTHORIZED);
                return prepareResponse(response);
            }
            ...
            if (workspaceService == null || !workspaceService.getIsHealthy()) {
                ...
                response.setErrorStatus("Could not check service availability.", HttpStatus.SC_INTERNAL_SERVER_ERROR);
                return prepareResponse(response);
            }
    ...
    public StatusMessageFormat checkWorkspace(String user) {
        ...
        try {
            ...
            response.setSuccessfulStatus();
            response.addMetadata("needsUpdate", !CoreService.WORKSPACE.getImage().equalsIgnoreCase(workspaceService.getDockerImage()));
            return prepareResponse(response);
        } catch (Exception e) {
            log.debug(e.getMessage(), e);
            // TODO do not return error -> currently this error comes if two check calls are called too fast after each other
            response.setSuccessfulStatus();
            return prepareResponse(response);
        }
    }
