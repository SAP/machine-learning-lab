...
public class LabApiHandler extends AbstractApiHandler<LabApiHandler> implements LabApi {
    ...
    public SingleValueFormat<LabProject> createProject(LabProjectConfig projectConfig) {
        ...
        try {
            if (projectConfig == null) {
                response.setErrorStatus("The project configuration in the body should not be empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }

            if (authProfile != null && AuthorizationManager.isProjectAdmin(authProfile.getId())) {
                response.setErrorStatus("A project admin user is not allowed to create projects.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            ...
            if (project == null) {
                response.setErrorStatus("Failed to create the project " + projectConfig.getName(), HttpStatus.SC_INTERNAL_SERVER_ERROR);
                return prepareResponse(response);
            }
    ...
    public StatusMessageFormat isProjectAvailable(String project) {
        ...
        try {
            if (projectManager.isProjectAvailable(project)) {
                ...
                response.setErrorStatus("Project name is not available for project creation.", HttpStatus.SC_INTERNAL_SERVER_ERROR);
            }
    ...
    public StatusMessageFormat deleteProject(String project) {
        ...
        try {
            if (authProfile != null && AuthorizationManager.isProjectAdmin(authProfile.getId())) {
                response.setErrorStatus("A project admin user is not allowed to delete projects.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            ...
            componentManager.getEventLogManager().logEvent(LabEvent.DELETE_PROJECT, new PropertyContainer()
    ...
    public ValueListFormat<LabProject> getProjects() {
        ...
        try {
            List<LabProject> filteredProjects =
                    authManager.filterProjects(authProfile.getPermissions(), componentManager.getProjectManager().getProjects());
    ...
    public SingleValueFormat<LabProject> getProject(String project, @Nullable Boolean expand) {
        ...
        try {
            ...
            if (expand != null && expand) {
                ...
                //labProject.setModels(componentManager.getFileManager().listRemoteFiles(project, LabFileDataType.MODEL));
                //labProject.setDatasets(componentManager.getFileManager().listRemoteFiles(project, LabFileDataType.DATASET));
    ...
    public StatusMessageFormat deleteService(String project, String service) {
        ...
        try {
            ...
            // log delete event
            componentManager.getEventLogManager().logEvent(LabEvent.DELETE_SERVICE, new PropertyContainer()
                    .addProperty(LabApi.PARAM_PROJECT, project)
                    .addProperty("user", authProfile != null ? authProfile.getId() : "unk"));
    ...
    public SingleValueFormat<String> getServiceLogs(String project, String service) {
        ...
        try {
            ...
            project = projectManager.resolveProjectName(project);
            String logs = serviceManager.getServiceLogs(serviceManager.getService(service, project).getDockerId());
    ...
    public ValueListFormat<LabFile> getFiles(String project, @Nullable LabFileDataType dataType, @Nullable String prefix, @Nullable Boolean aggregateVersions) {
        ...
        try {
            ...
            LabFileCollection fileCollection = componentManager.getFileManager().listRemoteFiles(project, dataType, prefix, aggregateVersions);
    ...
    public ValueListFormat<LabExperiment> getExperiments(String project) {
        ...
        try {
            ...
            List<LabExperiment> experiments = componentManager.getExperimentsManager().getExperiments(project);
    ...
    public StatusMessageFormat deleteExperiment(String project, String experimentKey) {
        ...
        try {
            if (StringUtils.isNullOrEmpty(experimentKey)) {
                response.setErrorStatus("The experiment parameter should not be empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            ...
            componentManager.getFileManager().deleteFolder(project, FileHandlerUtils.resolveKey(experimentKey, LabFileDataType.EXPERIMENT));

            // delete experiment
            componentManager.getExperimentsManager().deleteExperiment(project, experimentKey);

            // log delete experiment event
            componentManager.getEventLogManager().logEvent(LabEvent.DELETE_EXPERIMENT, new PropertyContainer()
                    .addProperty(LabApi.PARAM_PROJECT, project)
                    .addProperty("user", authProfile != null ? authProfile.getId() : "unk")
    ...
    public SingleValueFormat<String> syncExperiment(String project, LabExperiment experiment) {
        ...
        try {
            ...
            LabExperiment updatedExperiment = componentManager.getExperimentsManager().updateExperiment(project, experiment);
    ...
    @Override
    public SingleValueFormat<LabService> deployModel(String project, String modelKey,
        ...
        try {
            ...
            if (labFile == null) {
                response.setErrorStatus("Failed to deploy model. Model file not found.", HttpStatus.SC_NOT_FOUND);
                return prepareResponse(response);
            }
            ...
            if (StringUtils.isNullOrEmpty(name)) {
                ...
                name = StringUtils.simplifyKey(StringUtils.shorten(modelName, 62 - modelVersionSuffix.length())) + modelVersionSuffix;
            }
            ...
            log.error(e.getMessage(), e);
            response.setErrorStatus("Failed to deploy model as service", HttpStatus.SC_INTERNAL_SERVER_ERROR, e);
            return prepareResponse(response);
        }
    }
    ...
    public StatusMessageFormat deleteScheduledJob(String project, String jobId) {
        ...
        try {
            ...
            if (!componentManager.getJobManager().deleteScheduledJob(project, jobId)) {
                ...
                // log delete event
                componentManager.getEventLogManager().logEvent(LabEvent.DELETE_SCHEDULED_JOB, new PropertyContainer()
                        .addProperty(LabApi.PARAM_PROJECT, project)
                        .addProperty("user", authProfile != null ? authProfile.getId() : "unk"));
            }
    ...
    public StatusMessageFormat deleteJob(String project, String job) {
        ...
        try {
            ...
            // log delete event
            componentManager.getEventLogManager().logEvent(LabEvent.DELETE_JOB, new PropertyContainer()
                    .addProperty(LabApi.PARAM_PROJECT, project)
                    .addProperty("user", authProfile != null ? authProfile.getId() : "unk"));
    ...
    public SingleValueFormat<LabJob> deployJob(String project, String image, @Nullable String name, @Nullable String schedule, @Nullable Map<String, String> config) {
        ...
        try {
            ...
            if (!StringUtils.isNullOrEmpty(name) && !serviceManager.isValidServiceName(name)) {
                response.setErrorStatus(name + " is not a valid job name. The name has to conform the this regex: " + serviceManager.SERVICE_NAME_VALIDATION_REGEX, HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            ...
            if (!StringUtils.isNullOrEmpty(schedule)) {
                ...
                // run directly
                LabJob job = serviceManager.deployJob(serviceManager.createProjectJob(project, image, name).addEnvVariables(config));

                if (job != null) {
                    response.setSuccessfulStatus();
                    response.setData(job);
                } else {
                    response.setErrorStatus("Failed to deploy the job in project " + project, HttpStatus.SC_INTERNAL_SERVER_ERROR);
                }
            }
    ...
    public SingleValueFormat<LabService> deployService(String project, String image, @Nullable String name, @Nullable Map<String, String> config) {
        ...
        try {
            ...
            if (!StringUtils.isNullOrEmpty(name) && !serviceManager.isValidServiceName(name)) {
                response.setErrorStatus(name + " is not a valid service name. The name has to conform the this regex: " + serviceManager.SERVICE_NAME_VALIDATION_REGEX, HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }

            project = projectManager.resolveProjectName(project);

            DockerDeploymentConfig deploymentConfig = serviceManager.createProjectService(project, image, name).addEnvVariables(config);

            try {
                // check if service with same name already exists
                if (serviceManager.getService(deploymentConfig.getName()) != null) {
                    response.setErrorStatus("A service with the same name already exists.", HttpStatus.SC_BAD_REQUEST);
                    return prepareResponse(response);
                }
            } catch (Exception ex) {
                // do nothing
            ...
            if (service != null) {
                ...
                response.setErrorStatus("Failed to deploy the service in project " + project, HttpStatus.SC_INTERNAL_SERVER_ERROR);
            }
    ...
    @Override
    public StatusMessageFormat deleteFile(String project, String fileKey, @Nullable Integer keepLatestVersions) {
        StatusMessageFormat response = new StatusMessageFormat();

        try {
            if (StringUtils.isNullOrEmpty(fileKey)) {
                response.setErrorStatus("The fileKey parameter should not be empty.", HttpStatus.SC_BAD_REQUEST);
                return prepareResponse(response);
            }
            ...
            // log delete event
            componentManager.getEventLogManager().logEvent(LabEvent.DELETE_FILE, new PropertyContainer()
                    .addProperty(LabApi.PARAM_PROJECT, project)
                    .addProperty("user", authProfile != null ? authProfile.getId() : "unk")
    ...
    public SingleValueFormat<String> uploadFile(String project, InputStream fileStream, String fileName, LabFileDataType dataType, @Nullable Boolean versioning, @Nullable Map<String, String> metadata) {
        ...
        try {
            ...
            if (!metadata.containsKey(LabApi.FILE_METADATA_PREFIX + LabFile.META_CONTENT_TYPE)) {
                ...
                metadata.put(LabApi.FILE_METADATA_PREFIX + LabFile.META_CONTENT_TYPE, FileUtils.getContentType(FileHandlerUtils.removeVersionFromKey(fileName)));
            }

            String uploadedKey = componentManager.getFileManager().uploadFile(fileName, fileStream, dataType, project, versioning, metadata);
    ...
    public SingleValueFormat<LabFile> downloadFile(String project, String fileKey) {
        ...
        try {
            ...
            if (labFile == null) {
                response.setErrorStatus("Cannot find file " + fileKey + (project != null ? " in bucket: " + project : "")
                                                + " in " + project + " project.", HttpStatus.SC_NOT_FOUND);
                return prepareResponse(response);
            }

            // log download event
            componentManager.getEventLogManager().logEvent(LabEvent.DOWNLOADED_FILE, new PropertyContainer()
                    .addProperty(LabApi.PARAM_PROJECT, project)
                    .addProperty("user", authProfile != null ? authProfile.getId() : "unk")
    ...
    protected <F extends UnifiedFormat> F prepareResponse(F unifiedFormat) {
        ...
        try {
            if (!StringUtils.isNullOrEmpty(this.getRequestUrl()) && response.getMetadata() != null && response.getMetadata().getTime() != null) {
                if (response.getMetadata().getTime() > LabConfig.LOG_REQUEST_MAX_TIME) {
                    // request takes more than 10 seconds -> log event
                    componentManager.getEventLogManager().logEvent(LabEvent.LONG_REQUEST, new PropertyContainer()
