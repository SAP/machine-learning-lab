...
public class LabAdminEndpoint extends AbstractApiEndpoint<LabAdminEndpoint> {
    ...
    /**
     * Checks whether a research-workspace container for the passed id already exists. If not, a new one is created & started.
     *
     * @param id
    ...
    @ApiOperation(value = "Checks whether a workspace container for the passed id already exists. If not, a new one is created & started.", response = StatusMessageFormat.class)
    @Produces(MediaType.APPLICATION_JSON)
    @Pac4JSecurity(ignore = true)
    public Response checkWorkspace(@QueryParam(LabAdminApi.PARAM_WORKSPACE_ID) String id,
    ...
    @ApiOperation(value = "Shutdown all unused workspaces - 15 days without activity (admin-only).", response = ListOfLabUsers.class)
    @Produces(MediaType.APPLICATION_JSON)
    @Pac4JSecurity(clients = {
            AuthorizationManager.PAC4J_CLIENT_COOKIE,
    ...
    public Response shutdownUnusedWorkspaces(
            @ApiParam(value = "If 'true', it will only return candidates for shutdown.", defaultValue = "true", required = true) @QueryParam(LabAdminApi.PARAM_DRY_RUN) Boolean dryRun,
            @ApiParam(value = "Number of inactive days to consider workspace unused.", defaultValue = "14", required = false) @QueryParam(LabAdminApi.PARAM_DAYS_THRESHOLD) Integer daysThreshold,
            @ApiParam(value = "IDs to include as inactive users.", required = false) List<String> includedIds,
        ...
        return UnifiedResponseFactory.getResponse(adminApiHandler.shutdownUnusedWorkspaces(dryRun, daysThreshold, includedIds));
    }

    @PUT
    @Path(LabAdminApi.METHOD_SHUTDOWN_DISK_EXCEEDING_CONTAINERS)
    @ApiOperation(value = "Remove all workspaces that exceed the disk storage limit (docker-local mode only).", response = ListOfStringsResponse.class)
    @Pac4JSecurity(clients = {
        AuthorizationManager.PAC4J_CLIENT_COOKIE,
        AuthorizationManager.PAC4J_CLIENT_HEADER }, authorizers = AuthorizationManager.AUTHORIZER_ADMIN) 
    public Response shutdownDiskExceedingContainers(
        @ApiParam(value = "If 'true', it will only return candidates for removal.", defaultValue = "true", required = true) @QueryParam(LabAdminApi.PARAM_DRY_RUN) Boolean dryRun,
        @Pac4JProfile MongoProfile commonProfile,
        @BeanParam DefaultHeaderFields defaultHeaders) {
        if (dryRun == null) {
            dryRun = true;
        ...
        adminApiHandler.setAuthProfile(commonProfile);
        return UnifiedResponseFactory.getResponse(adminApiHandler.shutdownDiskExceedingContainers(dryRun));
    }


    @PUT
    @Path(LabAdminApi.METHOD_RESET_ALL_WORKSPACES)
    @ApiOperation(value = "Resets all workspaces. Use with caution (admin-only).", response = StatusMessageFormat.class)
    @Produces(MediaType.APPLICATION_JSON)
    @Pac4JSecurity(clients = {
            AuthorizationManager.PAC4J_CLIENT_COOKIE,
            AuthorizationManager.PAC4J_CLIENT_HEADER }, authorizers = AuthorizationManager.AUTHORIZER_ADMIN)
    public Response resetAllWorkspaces(@Pac4JProfile MongoProfile commonProfile,
                                       @BeanParam DefaultHeaderFields defaultHeaders) {
        adminApiHandler.setAuthProfile(commonProfile);
    ...
    @ApiOperation(value = "Resets a workspace. Removes the container (keeps all persisted data) and starts a new one.", response = LabServiceResponse.class)
    @Produces(MediaType.APPLICATION_JSON)
    @Pac4JSecurity(clients = {
            AuthorizationManager.PAC4J_CLIENT_COOKIE,
            AuthorizationManager.PAC4J_CLIENT_HEADER }, authorizers = AuthorizationManager.AUTHORIZER_IS_AUTHENTICATED)
    public Response resetWorkspace(@QueryParam(LabAdminApi.PARAM_WORKSPACE_ID) String id,
                                   @Pac4JProfile MongoProfile commonProfile,
                                   @BeanParam DefaultHeaderFields defaultHeaders) {
        adminApiHandler.setAuthProfile(commonProfile);
    ...
    @ApiOperation(value = "Returns statistics about this Lab instance (admin-only).", response = LabStatisticsResponse.class)
    @Produces(MediaType.APPLICATION_JSON)
    @Pac4JSecurity(clients = {
            AuthorizationManager.PAC4J_CLIENT_COOKIE,
            AuthorizationManager.PAC4J_CLIENT_HEADER }, authorizers = AuthorizationManager.AUTHORIZER_ADMIN)
    ...
    @ApiOperation(value = "Returns information about this Lab instance.", response = LabInfoResponse.class)
    @Produces(MediaType.APPLICATION_JSON)
    @Pac4JSecurity(ignore = true)
    ...
    @ApiOperation(value = "Returns events filtered by a specified event type (admin-only).", response = ListOfLabEventsResponse.class)
    @Produces(MediaType.APPLICATION_JSON)
    @Pac4JSecurity(clients = {
            AuthorizationManager.PAC4J_CLIENT_COOKIE,
            AuthorizationManager.PAC4J_CLIENT_HEADER }, authorizers = AuthorizationManager.AUTHORIZER_ADMIN)
    public Response getEvents(
            @ApiParam(value = "Event Type. If not provided, all events will be returned.", required = false) @QueryParam(LabAdminApi.PARAM_EVENT) String event,
            @BeanParam DefaultHeaderFields defaultHeaders) {
