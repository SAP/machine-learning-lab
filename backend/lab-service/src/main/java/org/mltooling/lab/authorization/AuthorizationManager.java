 ...
 * <p>
 * user = user-id or user-name , userName = user-name only, userId = user-id only
 */
public class AuthorizationManager {
    ...
    public static final int JWT_EXPIRATION_TIME_WEEK_IN_SECONDS = JWT_EXPIRATION_TIME_DAY_IN_SECONDS * 7;
    public static final int JWT_API_TOKEN_EXPIRATION_TIME_YEAR_IN_SECONDS = JWT_EXPIRATION_TIME_WEEK_IN_SECONDS * 500; // 500 weeks -> 10 years
    ...
    @Deprecated
    private static final String PROJECT_USER_PREFIX_COMPAT = "project-admin-"; // only for compatibility reasons
    ...
    // min chars 2 , max chars 40 - added min max length for project and user name validation (to conform with Minio)
    private static final String USER_NAME_VALIDATION_REGEX = "^[a-zA-Z0-9][a-zA-Z0-9\\-]{1,38}[a-zA-Z0-9]$"; // TODO do not allow spaces for now
    ...
    /**
     * Creates a user profile and stores it in the database. Based on that profile, all JWT token will be generated.
     *
     ...
     */
    public MongoProfile createUser(String userName,
                                   String password) throws Exception {
        return createUser(userName, password, null, null);
    }

    /**
     * Create an admin profile. Therefore, the passed secret must match the server's secret.
     * If the secret matches, the method {@link LabAuthApiHandler#createUser} is called and its value returned, otherwise an Exception is thrown.
     *
     * @param jwtSecret must match the server's secret
     * @return the created profile with admin permission based on return value of {@link LabAuthApiHandler#createUser}
     * @throws UnauthorizedException if the passed jwtSecret does not match the server's secret
     */
    public MongoProfile createAdminUser(String userName, String password, String jwtSecret) throws Exception {
        if (!StringUtils.isNullOrEmpty(jwtSecret) && jwtSecret.equals(LabConfig.JWT_SECRET)) {
            return createUser(userName, password, Collections.singleton(CORE_PERMISSIONS.ADMIN.getName()), null);
        }
    ...
    /**
     * Same as {@link AuthorizationManager#createUser(String, String, Set, Map)} but with random password.
     * Can be used to create a technical user profile.
     */
    public MongoProfile createUser(String userName,
                                   @Nullable Set<String> permissions,
                                   @Nullable Map<String, Object> attributes) throws Exception {
        return createUser(userName, generateSecureRandomString(), permissions, attributes); // create the API token with a random password
    }
     ...
     * @param permission the permission to be added. Will overwrite an existing permission with same name.
     */
    ...
    public void removePermissionsFromAllProfiles(Set<String> permissions) throws Exception {
        FindIterable<Document> profileIterator = getLabMongoDb().getCollection(USERS_COLLECTION_NAME).find();
    ...
    /**
     * Overwrites all existing permissions with the passed set of permissions. If needed, first fetch all user permissions, alter the list and
     * send the new list here. If true, the refresh token for the profile can be changed.
     * Doing so will let existing short-term tokens expire without the possibility to refresh them without new loginUser.
     *
     * @param user               of the profile to be updated
     * @param permissions        set of all permissions the profile will contain
     * @param changeRefreshToken if true, the refresh token in the database will be changed. JWT tokens issued before the modification cannot be renewed without a new loginUser.
     */
    public void updatePermissions(String user, List<String> permissions, @Nullable Boolean changeRefreshToken) throws Exception {
    ...
    /**
     * Update the password of a given user. THe refresh token will be changed so that the user has to re-login the next time.
     *
     * @param user               of the profile to be updated
     * @param password           new password
     * @param changeRefreshToken if true, the refresh token in the database will be changed. JWT tokens issued before the modification cannot be renewed without a new loginUser.
     */
    public void updatePassword(String user, String password, @Nullable Boolean changeRefreshToken) throws Exception {
        if (changeRefreshToken == null) {
            changeRefreshToken = false;
        }
    ...
    public String createApiToken(String user) throws Exception {
        return createJwtToken(getUser(user), JWT_API_TOKEN_EXPIRATION_TIME_YEAR_IN_SECONDS, TOKEN_TYPES.LONG);
    }
    ...
    public String createProjectToken(String project) throws Exception {
        ...
        if (projectUser == null) {
            ...
            projectUser = createUser(PROJECT_USER_PREFIX + project, new HashSet<>(Arrays.asList(ProjectAuthorizer.prefixProjectNameIfNecessary(project))), null);
        }

        return createJwtToken(projectUser, JWT_API_TOKEN_EXPIRATION_TIME_YEAR_IN_SECONDS, TOKEN_TYPES.LONG);
    }
    ...
    public String createAdminToken() throws Exception {
        ...
        if (technicalAdmin == null) {
            ...
            technicalAdmin = createAdminUser(TECHNICAL_ADMIN_USER, generateSecureRandomString(), LabConfig.JWT_SECRET);

        }

        return createJwtToken(technicalAdmin, JWT_API_TOKEN_EXPIRATION_TIME_YEAR_IN_SECONDS, TOKEN_TYPES.LONG);
    }
    ...
    /**
     * If the refreshToken in the profile is valid (incoming refresh token == refresh token stored in db),
     * issue a new JWT token with new expiration time.
     *
    ...
    public String refreshToken(CommonProfile commonProfile) {
        ...
        if (commonProfile.getAttribute(REFRESH_TOKEN_ATTRIBUTE).equals(
                savedProfile.getAttribute(REFRESH_TOKEN_ATTRIBUTE))) {
            return createAppToken(savedProfile);
        }

        log.info("Refresh token of authorized profile (" + commonProfile.getId() + ") is not equal to saved profile (" + savedProfile.getId() + ").");
        return null;
    }
    ...
    public List<String> getProfileIds() {
        ...
        FindIterable<Document> userIterator = getLabMongoDb().getCollection(USERS_COLLECTION_NAME).find();
    ...
    /**
     * Create a Jwt Token based on the given profile and the passed attributes. The token will be signed using the secret stored in {@link LabConfig#JWT_SECRET}.
     *
     * @param profile                 Based on which the token will be generated
     * @param expirationTimeInSeconds Defines when the token will expire. An expired token will be automatically rejected upon all authorization checks.
     * @param tokenType               Specifies the type of the token. Will be stored within the token itself. Tokens of type {@link AuthorizationManager.TOKEN_TYPES#LONG} will not contain any permissions, hence a db lookup has to be done for them.
     * @return the generated Jwt Token
     */
    public <T extends CommonProfile> String createJwtToken(T profile, int expirationTimeInSeconds, TOKEN_TYPES tokenType) {
        JwtGenerator<T> generator = new JwtGenerator<>(new SecretSignatureConfiguration(LabConfig.JWT_SECRET));
    ...
    public String createTechnicalToken(CommonProfile commonProfile) throws Exception {
        MongoProfile profile = createUser(commonProfile.getId(), commonProfile.getPermissions(), commonProfile.getAttributes());
    ...
    public MongoProfile createUser(String userName,
        ...
        if (!isValidUserName(userName)) {
            throw new Exception("The user name is not valid. It may only contain letters, digits, spaces, and the minus-character.");
        }
    ...
    private boolean userExists(String userId) {
        try {
            return getLabMongoDb().getCollection(USERS_COLLECTION_NAME).countDocuments(new Document(USER_ID_PROPERTY, userId)) > 0;
        } catch (Exception e) {
    ...
    public Config getPac4jConfig() {
        ...
        mongoProfileService.setUsernameAttribute(USER_NAME_ATTRIBUTE); // username attribute has to be set as demanded by Pac4J
        mongoProfileService.setPasswordEncoder(new ShiroPasswordEncoder(new DefaultPasswordService())); // used to encrypt the profile passwords in the database

        // The authClient checks basic auth credentials of the user stored in MongoDB and, if valid, returns the profile
        ...
        if (LabConfig.JWT_SECRET.equals(DEFAULT_JWT_SECRET)) {
            log.warn("SECURITY RISK: ML Lab was started with the default jwt secret. "
        ...
        Authenticator jwtAuthenticator = new LocalCachingAuthenticator<>(
                new JwtAuthenticator(new SecretSignatureConfiguration(LabConfig.JWT_SECRET)), 100, 15, TimeUnit.MINUTES);
        ...
        // Note: long-term api tokens don't have access to ADMIN annotated methods, as the permission itself is not in the JWT token and must be looked up in the DB, which doesn't happen with the
        // 'RequireAnyPermissionAuthorizer'. If this should change, a custom authorizer must be written.
        config.addAuthorizer(AUTHORIZER_ADMIN, new AdminAuthorizer()); //new RequireAnyPermissionAuthorizer(CORE_PERMISSIONS.ADMIN.getName()));
    ...
    public void configureSecuritySettings(ResourceConfig resourceConfig) {
        ...
        resourceConfig.register(new JaxRsConfigProvider(config))
        ...
        try {
            ...
            log.info("Admin user created with following password " + password +
                             "\nApp Token (short-term): " + token +
                             "\nAPI Token (long-term): " + authorizationApi.createApiToken(DEFAULT_ADMIN_USER).getData());
        } catch (Exception e) {
    ...
    public static String resolveUserName(String user) throws Exception {
        if (!isValidUserName(user)) {
            ...
            throw new Exception("The user name " + user + " is not valid. It may only contain letters, digits, spaces, and the minus-character.");
        }
    ...
    public static MongoProfile transformSerializedProfile(Document user) {
        return (MongoProfile) javaSerializationHelper.unserializeFromBase64(user.getString("serializedprofile"));
    }
    ...
    public List<LabUser> getInactiveUsers(@Nullable Integer daysThreshold) {
        ...
        for (MongoProfile user : getUsers()) {
            try {
                ...
                LabFile workspaceMetadataFile = ComponentManager.INSTANCE.getFileManager().getFile("backups/metadata.json", userProject);
