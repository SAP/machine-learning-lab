...
public class Launcher {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (LabConfig.LAB_ACTION == LabConfig.LabAction.SERVE) {
      ...
      System.exit(0);
    } else if (LabConfig.LAB_ACTION == LabConfig.LabAction.INSTALL
        || LabConfig.LAB_ACTION.isUnknown()) {
  ...
  private static void startService(@Nullable Integer port) {
    ...
    ServerConfig serverConfig =
        new ServerConfig(LabEndpoint.class.getPackage().toString())
            .setBaseUrl(LabConfig.LAB_BASE_URL)
            .setServicePort(port)
            .setLoggingActivated(true)
            .setApiDocsEndpoint(API_DOCS_ENDPOINT_PATH)
            .setServerCallback(
                new DefaultServerFactory.ServerCallback() {

                  @Override
                  public void onStart(HttpServer server) {

                    HttpHandler documentationHandler =
                        new CLStaticHttpHandler(ClassLoader.getSystemClassLoader(), "/docs/");
                    // server.getServerConfiguration().addHttpHandler(documentationHandler,
                    // LAB_DOCS_ENDPOINT_PATH); // TODO add trailing slash with new libs?
                    server
                        .getServerConfiguration()
                        .addHttpHandler(
                            documentationHandler,
                            HttpHandlerRegistration.builder()
                                .contextPath(LAB_DOCS_ENDPOINT_PATH)
                                .urlPattern("/")
                                .build());

                    HttpHandler webappHandler =
                        new CLStaticHttpHandler(ClassLoader.getSystemClassLoader(), "/app/");
                    server
                        .getServerConfiguration()
                        .addHttpHandler(
                            webappHandler,
                            HttpHandlerRegistration.builder()
                                .contextPath(WEBAPP_ENDPOINT_PATH)
                                .urlPattern("/")
                                .build());
                    // server.getServerConfiguration().addHttpHandler(webappHandler,
                    // WEBAPP_ENDPOINT_PATH); // TODO add trailing slash with new libs?
                  ...
                  public void onInit(ResourceConfig resourceConfig) {
                    ComponentManager.INSTANCE
                        .getAuthManager()
                        .configureSecuritySettings(resourceConfig);
                  }
