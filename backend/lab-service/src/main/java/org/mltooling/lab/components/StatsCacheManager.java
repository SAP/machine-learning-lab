...
public class StatsCacheManager {
    ...
    public LabProjectsStatistics getStats() {
        ...
        if ((new Date().getTime() - labStatsCache.getCacheUpdateDate().getTime()) > MAX_CACHE_VALIDITY) {
    ...
    public void updateCache() {
        ...
        labStatsCache.setDownloadedFiles(componentManager.getEventLogManager().getEvents(LabEvent.DOWNLOADED_FILE).size());
        ...
        for (LabProject project : projects) {
            try {
                ...
                if (projectCache.getExperimentsCount() != null) {
                    labStatsCache.setExperimentsCount(labStatsCache.getExperimentsCount() + projectCache.getExperimentsCount());
                }

                if (projectCache.getServicesCount() != null) {
                    labStatsCache.setServicesCount(labStatsCache.getServicesCount() + projectCache.getServicesCount());
                }
                ...
                if (projectCache.getModelsCount() != null) {
                    labStatsCache.setModelsCount(labStatsCache.getModelsCount() + projectCache.getModelsCount());
                }

                if (projectCache.getModelsTotalSize() != null) {
                    labStatsCache.setModelsTotalSize(labStatsCache.getModelsTotalSize() + projectCache.getModelsTotalSize());
                }

                if (projectCache.getDatasetsCount() != null) {
                    labStatsCache.setDatasetsCount(labStatsCache.getDatasetsCount() + projectCache.getDatasetsCount());
                }

                if (projectCache.getDatasetsTotalSize() != null) {
                    labStatsCache.setDatasetsTotalSize(labStatsCache.getDatasetsTotalSize() + projectCache.getDatasetsTotalSize());
                }
                ...
                if (projectCache.getFilesTotalSize() != null) {
                    labStatsCache.setFilesTotalSize(labStatsCache.getFilesTotalSize() + projectCache.getFilesTotalSize());
                }
            } catch (Exception ex) {
                log.warn("Failed to get statistics for project: " + project + "; Exception: " + ex.getMessage());
            }
    ...
    public void updateCache(String project) {
        try {
            ...
            perfLogger.start();
            LabFileCollection datasetsCollection = componentManager.getFileManager().listRemoteFiles(project, LabFileDataType.DATASET);
            statistics.setDatasetsCount(datasetsCollection.getLabFiles().size()); // use count of datasets instead of file count
            statistics.setDatasetsTotalSize((double) datasetsCollection.getTotalSize());

            try {
                statistics.setUserCount(componentManager.getProjectManager().getProjectMembers(labProject.getId()).size());
            } catch (Exception ex) {
                // do nothing
            }
            ...
            perfLogger.start();
            LabFileCollection modelsCollection = componentManager.getFileManager().listRemoteFiles(project, LabFileDataType.MODEL);
            statistics.setModelsCount(modelsCollection.getLabFiles().size()); // use count of models instead of file count
            ...
            try {
                ...
                perfLogger.start();
                statistics.setServicesCount(componentManager.getServiceManger().getServices(project).size());
