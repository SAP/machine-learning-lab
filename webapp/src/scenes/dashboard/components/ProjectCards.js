...
class ProjectCard extends Component {
  ...
  render() {
    ...
    return (
      <Grid item xs={12} sm={12} md={6} lg={this.props.colSize} className={classes.gridItem}>
          ...
          <div className={classes.details}>
            <CardHeader
              ...
              avatar={
                <Avatar
                  ...
                  alt={this.props.name}
                  // src={imageUrl}
                  children={<svg ref={this.svgRef} width="60" height="60" style={{backgroundColor: "white"}} data-jdenticon-value={this.props.name} />}
                />
              }
            ...
            <CardActions className={classes.controls}>
              <Button
                size="small"
                target="_blank"
                onClick={() => this.props.onSelectProject(this.props.name, this.props.item.id)}
              >
                SELECT
              </Button>
              <Button
              ...
              <Button
                size="small"
                target="_blank"
                onClick={() => this.props.onGetApiToken(this.props.name, this.props.item.id)}
              >
                TOKEN
              </Button>
              <Button
...
class ProjectCards extends Component {
  constructor(props) {
    ...
    this.state = {
      renderedItems: 0,
      sortedData: (props.data !== null) ? props.data.sort((a, b) => (a.name > b.name ? 1 : b.name > a.name ? -1 : 0)).map(item => item) : [],
      projectCards: []
    }
  ...
  componentDidUpdate(prevProps) {
    if ((this.props.data !== null && prevProps.data !== null && 
      ...
      const sortedData = this.props.data.sort((a, b) => (a.name > b.name ? 1 : b.name > a.name ? -1 : 0));
      this.setState({sortedData: sortedData, projectCards: [], renderedItems: 0});
        ...
        this.increaseRenderedItemsCountTimer = window.requestAnimationFrame(() => {
          ...
          this.setState({projectCards: [...this.state.projectCards, projectCards], renderedItems: this.state.renderedItems + this.renderItemsChunkSize});
        });
    }
  }

  getProjectCards() {
    return this.state.sortedData.slice(this.state.renderedItems, this.state.renderedItems + this.renderItemsChunkSize).map((item, index) => {
      return (
        <ProjectCard
