...
import { authorizationApi, getDefaultApiCallback, toastErrorMessage, toastSuccess } from "../../../services/client/ml-lab-api";
			...
			getDefaultApiCallback(
				() => {
					...
					this.setState({
						...
						userNameOptions: this.state.userNameOptions.filter(option => option.value !== user)
					});
				},
		...
		authorizationApi.removeUserFromProject(
			...
			getDefaultApiCallback(
				() => {
					...
					let modifiedProjectMembers = this.state.projectMembers.filter(member => member !== user);
					this.setState({
						...
						userNameOptions: [...this.state.userNameOptions, { value: user, label: user }]
					});
				},
			...
			<div className={classes.root}>
				<div style={{ display: "flex", padding: "24px"}}>
					<div style={{ width: "50%", display: "inline-block", margin: "4px" }}>
						...
						<List className={classes.list} style={{ backgroundColor: "#b5b5b514", borderRadius: "6px", padding: "8px", margin: "8px" }}>
					...
					<div style={{ width: "50%", display: "inline-block", paddingLeft: "8px" }}>
						<Typography variant="subtitle1" style={{ color: "grey", fontSize: 13 }}>
							Add User:
						</Typography>
				...
				<DialogActions>
					<Button size="small" style={{ width: "100px" }} color="primary" onClick={this.state.requestClose}>
						{" "}
						Cancel
					</Button>
					<Button
	...
	render() {
		...
		return(
			<Select 
				...
				value={memberToAdd} onChange={this.props.onChange} options={options} styles={this.customStyles} 
			/>
		);
	}
}
