...
import { makeAllUpperCaseAndAddWhitespace, DurationFormatter, secondsToDateAndTime, NumberFormatter } from "./grid-formatter";
 ...
 */
let specialColumns = {
    "status":  {field: "status", title: makeAllUpperCaseAndAddWhitespace("Status"), cellStyle: {minWidth: 160}, render: renderStatusCell },
    "name": {field: "name", title: makeAllUpperCaseAndAddWhitespace("Name"), cellStyle: {minWidth: 300}},
    "key": {field: "key", title: makeAllUpperCaseAndAddWhitespace("Key"), hidden: false, cellStyle: {minWidth: 300}},
    "project": {field: "project", title: makeAllUpperCaseAndAddWhitespace("Project"), hidden: false, cellStyle: {width: 160}},
    "operator": {field: "operator", title: makeAllUpperCaseAndAddWhitespace("Operator"), cellStyle: {minWidth: 160}},
    "scriptName": {field: "scriptName", title: makeAllUpperCaseAndAddWhitespace("Script Name"), hidden: false, cellStyle: {minWidth: 160}},
    "scriptType": {field: "scriptType", title: makeAllUpperCaseAndAddWhitespace("Script Type"), hidden: false, cellStyle: {minWidth: 160}},
    "startedAt": {field: "startedAt", title: makeAllUpperCaseAndAddWhitespace("startedAt"), type: 'date', render: rowData => secondsToDateAndTime(rowData.startedAt), defaultSort: "desc", cellStyle: {minWidth: 160}},
    "finishedAt": {field: "finishedAt", title: makeAllUpperCaseAndAddWhitespace("finishedAt"), type: 'date', render: rowData => secondsToDateAndTime(rowData.finishedAt), cellStyle: {minWidth: 160}},
    "updatedAt": {field: "updatedAt", title: makeAllUpperCaseAndAddWhitespace("updatedAt"), type: 'date', hidden: false, render: rowData => secondsToDateAndTime(rowData.updatedAt), cellStyle: {minWidth: 160}},
    "duration": {field: "duration", title: makeAllUpperCaseAndAddWhitespace("duration"), render: rowData => DurationFormatter({value: rowData.duration}), cellStyle: {minWidth: 160}},
    "command": {field: "command", title: makeAllUpperCaseAndAddWhitespace("Command"), hidden: false, cellStyle: {minWidth: 160}},
    "clientVersion": {field: "clientVersion", title: makeAllUpperCaseAndAddWhitespace("clientVersion"), hidden: false, cellStyle: {minWidth: 160}},
    "result": {field: "result", title: makeAllUpperCaseAndAddWhitespace("Result"), render: rowdata => NumberFormatter({value: rowdata.result}), cellStyle: {minWidth: 160}}
}
let initiallyVisibleColumns = ["status", "name", "operator", "startedAt", "finishedAt", "updatedAt", "duration", "result"];
...
const customFilterAndSearch = (field, parentField) => {
    return (term, rowdata) => {
        const fieldValue = (parentField) ? rowdata[parentField][field] : rowdata[field];
...
class ExperimentsNew extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            visibleColumns: initiallyVisibleColumns.map((key) => {return {...specialColumns[key]}}), // make a copy of the column specifications because material-table modifies the objects directly
    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.props.currentProject !== prevProps.currentProject 
            || this.state.jsonExperiments.data.length !== prevState.jsonExperiments.data.length) {
            this.loadExperiments();
     ...
     */
    loadExperiments = () => {
        GridApiHelper.getModifiedExperiments(this.props.currentProject, this.state.widgetdata).then(
            (resultDict) => { 
                this.setState({
            ...
            (error) => {
                console.log("componentDidUpdate:callback of set state of processExperiments failed", error);
            }
        );
    }
                ...
                if (typeof value === "object" && !Array.isArray(value)) {
                    ...
                    for (const subAttributeName of subAttributeNames) {
                        ...
                        attributeColumns[attributeName][subAttributeName] = {
                            attribute: subAttributeName, visible: false
                        };
                    }
                } else {
                    ...
                    attributeColumns[OVERVIEW_KEY][attributeName] = {
                        attribute: attributeName, visible: isVisible
                    };
                }
            }
        }
    ...
    onAttributeActivatedChange = (attributes) => {
        ...
        for (let key of Object.keys(specialColumns)) {
            if (copiedAttributes[OVERVIEW_KEY][key] && 
        ...
        for (let attributeCategory of Object.keys(copiedAttributes)) {
            ...
            for (let attribute of Object.values(copiedAttributes[attributeCategory])) {
                if (attribute.visible === true) {
                    const parentField = (attributeCategory !== OVERVIEW_KEY) ? attributeCategory : null;
                    let visibleColumn = getDefaultMaterialTableColumn(attribute.attribute, parentField);
                    visibleColumns.push(visibleColumn);
                }
            }
        }
        
        this.setState({visibleColumns: visibleColumns, columnsConfiguration: attributes});
    }
    ...
    render() {
        ...
        const ActionComponent = (props) => {
            ...
            return <div style={{display: "inline-flex"}} {...props}>{IconComponent}</div>;
        }
            ...
            <div style={{ width: "100%" }}>
                ...
                <Paper>
                    <MaterialTable
                        ...
                        detailPanel={[{
                            render: rowData => {return this.renderDetailPanel(rowData)},
                            tooltip: "Details"
                        }]}
                        options={{
                            filtering: true,
                                ...
                                icon: (props) => (
                                    <DeleteExperimentButton
                                        ...
                                        onItemDelete={() => {
                                            ...
                                            let toastID = ProcessToast.showProcessToast("Experiment will be deleted...");

                                            GridApiHelper.deleteExperiment(project, experimentKey).then(() => {
