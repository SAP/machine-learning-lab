...
import { getFileDownloadUrl, toastInfo } from "../../services/client/ml-lab-api";
...
class RowDetailPanel extends React.PureComponent {
	...
	render() {
		...
		return (
			<div style={{ display: "flex", flexGrow: 1, marging: "0px", padding: "0px !important" }}>
				...
				{activeIndex === 0 && (
					<TabContainer>
						<OverviewTable experimentsAttributes={this.props.experiments} data={this.props.experiment} />
					</TabContainer>
				)}
				{activeIndex === 1 && (
					<TabContainer>
				...
				{activeIndex === 7 && (
					<TabContainer>
						<FilesTable currentProject={this.props.currentProject} data={this.props.experiment} />
					</TabContainer>
				)}
			</div>
		);
...
function TabContainer(props) {
	return (
		<Typography component="div" style={{ flex: 1, padding: 30, float: "right", justifyContent: "right" }}>
			{props.children}
		</Typography>
	);
}
		...
		if (resources[type] !== undefined) {
			if (Array.isArray(resources[type])) {
				resources[type].forEach(element => {
                    rows.push({type: type, value: element, downloadUrl: getFileDownloadUrl(props.currentProject, element)})
				});
			} else {
                rows.push({ type: type, value: resources[type], downloadUrl: getFileDownloadUrl(props.currentProject, resources[type])});
			}
		}
	});
			...
			<Paper>
                <MaterialTable 
                    ...
                    actions={[
                        rowData => ({
                            icon: (props) => (
                                <CopyKeyButton key="copyBtn" copyKey={rowData.value} onCopy={onCellClickCopyKey} />
                            )
                        }),
                        rowData => ({
                            icon: (props) => (<DownloadItemButton
                                key="downloadBtn"
