...
SSH_TARGET_PUBLICKEY_API_PORT = int(os.getenv("SSH_TARGET_PUBLICKEY_API_PORT", 8080))
...
def get_authorized_keys_kubernetes(query_cache: list = []) -> (list, list):
    ...
    for pod in pod_list.items:
        ...
        try:
            ...
            print(
                "Connection to {ip} timed out after {timeout} seconds. Will try to exec into the pod to retrieve the key.".format(
                    ip=pod_ip, timeout=str(timeout_seconds)))
        ...
        if key is None:
            try:
                exec_result = stream.stream(kubernetes_client.connect_get_namespaced_pod_exec, name,
                                            NAMESPACE, command=PRINT_KEY_COMMAND, stderr=True, stdin=False, stdout=True,
                                            tty=False)
                key = exec_result
            except:
...
def get_authorized_keys_docker(query_cache: list = []) -> (list, list):
    ...
    for container in containers:
        ...
        try:
            ...
            print(
                "Connection to {ip} timed out after {timeout} seconds. Will try to exec into the pod to retrieve the key.".format(
                    ip=container.id, timeout=str(timeout_seconds)))
